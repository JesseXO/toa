...
2. Extend user body profile fields in [backend/src/models/User.ts](../src/models/User.ts) with heightCm, weightKg, bmi, chestCm, waistCm, hipCm, inseamCm, shoulderCm, bodyShape, preferredFit, onboardingCompletedAt, recommendationVersion.
3. Extend product sizing metadata in [backend/src/models/Product.ts](../src/models/Product.ts) with sizeChartJson, fitType, recommendationTags, and accessory-specific fit hints.
4. Add explicit migrations and stop relying on runtime alter-sync behavior in [backend/src/index.ts](../src/index.ts). This blocks safe rollout.
5. Expand launch-ready seed data in [backend/src/seed-data.ts](../src/seed-data.ts) to include realistic size charts and fit metadata for all current categories.
...
8. Add user onboarding/profile endpoints in [backend/src/routes/userRoutes.ts](../src/routes/userRoutes.ts) and new user resolvers:
...
13. Extend product recommendation output in [backend/src/controllers/product/getProductByIdResolver.ts](../src/controllers/product/getProductByIdResolver.ts) and route registration in [backend/src/routes/productRoutes.ts](../src/routes/productRoutes.ts), plus optional POST /products/:id/recommend-size.
...
16. Extend user state model and actions in [app/src/store/user/user.types.js](../../app/src/store/user/user.types.js), [app/src/store/user/user.action.js](../../app/src/store/user/user.action.js), [app/src/store/user/user.reducer.js](../../app/src/store/user/user.reducer.js), and [app/src/store/user/user.saga.js](../../app/src/store/user/user.saga.js) for onboarding lifecycle + recommendation payloads.
17. Add FE network methods in [app/src/store/network/user.js](../../app/src/store/network/user.js) for infer/profile/recommendation APIs.
18. Update signup handoff in [app/src/components/sign-up-form/sign-up-form.component.jsx](../../app/src/components/sign-up-form/sign-up-form.component.jsx) and route orchestration in [app/src/App.js](../../app/src/App.js) to redirect first-time users into onboarding.
19. Implement multi-step onboarding UI (camera capture + file fallback + manual edit + summary + completion) from auth entry in [app/src/routes/authentication/authentecation.component.jsx](../../app/src/routes/authentication/authentecation.component.jsx).
20. Phase 4: Product integration + full visual redesign.
21. Add recommendation badge and preselected size behavior in [app/src/components/quick-view/quick-view.component.jsx](../../app/src/components/quick-view/quick-view.component.jsx), with manual override preserved.
22. Add category-level recommendation surfaces in shop route/component tree rooted at [app/src/routes/shop/shop.component.jsx](../../app/src/routes/shop/shop.component.jsx).
23. Full UI redesign pass across auth, onboarding, home, navigation, shop/category, product quick view/detail, and checkout using and extending [app/src/styles/theme.js](../../app/src/styles/theme.js) for consistency.
...
4. FE checks in [app/package.json](../../app/package.json): npm run build and npm test.
5. BE runtime checks in [backend/package.json](../package.json): npm start plus endpoint smoke tests (no automated test suite currently).
