# TOA Fit-Aware Commerce — Changes Report
...
```js
// app/src/routes/onboarding/inference/landmarks.js
...
The live loop in [`capture-step.jsx`](../../app/src/routes/onboarding/steps/capture-step.jsx) is structured to be cancel-safe and back-pressure-aware:
...
The math in [`inference/measurements.js`](../../app/src/routes/onboarding/inference/measurements.js) bridges abstract keypoint space to centimeters using the user's height as the scale anchor:
...
A second client-side inference pipeline runs on the same captured frame: [`utils/style-inference.js`](../../app/src/utils/style-inference.js). It downsamples the image to 64×64, bins each pixel into a 4×4×4 RGB cube (64 buckets), takes the top 5 most-populated buckets, averages each, and converts to hex. That's the user's dominant palette.
...
A new wizard step ([`preferences-step.jsx`](../../app/src/routes/onboarding/steps/preferences-step.jsx)) with two 4×2 tile grids (responsive 2×4 on mobile):
...
[`preferences-options.js`](../../app/src/routes/onboarding/preferences-options.js) is the single source of truth for these IDs, descriptions, and recommendation-tag mappings. Adding a 9th style is a one-line addition. The BE validator's allowlists mirror these IDs.
...
[`styles/style-helpers.js`](../../app/src/styles/style-helpers.js) exports tiny accessors so per-component refactors stayed short:
...
Old shop: `<CategoriesPreview />` over `<Category />` with a non-functional filter modal and a Floating Action Button. New shop ([`routes/shop/`](../../app/src/routes/shop/)):
...
New route `/product/:productId` — [`routes/product-detail/`](../../app/src/routes/product-detail/). Layout: hero gallery left, info right (title, price, description, fit-type pill, recommended-size pill, style-match percentage), size picker (recommended size is starred), full size chart with the active row highlighted, and a **"Why we recommend size X" panel** showing per-measurement deltas:
...
[`cart-item.component.jsx`](../../app/src/components/cart-item/cart-item.component.jsx) and [`checkout-item.component.jsx`](../../app/src/components/checkout-item/checkout-item.component.jsx) — show "✓ your fit" or warning "recommended M" chips next to the selected size based on a real-time `getRecommendedSize(cartItem, bodyProfile)` call.

[`checkout.component.jsx`](../../app/src/routes/checkout/checkout.component.jsx) — the order summary now aggregates fit status: "All N match your fit ✓" or "X match · Y differ from recommendation". The whole checkout page was also restructured into a two-column desktop layout (items left, sticky summary + payment right) that stacks cleanly on mobile.

### 4.12 Navigation Pass — Reworked

[`navigation.styles.jsx`](../../app/src/routes/navigation/navigation.styles.jsx) + [`navigation.component.jsx`](../../app/src/routes/navigation/navigation.component.jsx):
...
- [`search-bar.styles.jsx`](../../app/src/components/search-bar/search-bar.styles.jsx) — `SearchResults` is now flowing inline below the input (relative position), with z-index bumped to 1700.
- [`navigation.styles.jsx`](../../app/src/routes/navigation/navigation.styles.jsx) + component — split the modal into a fixed `SearchModalCard` shell and an inner `SearchModalBody` that owns the scrolling. Header stays pinned, results scroll with the body.

Mobile filter drawer redesign in the new shop ([`shop.styles.jsx`](../../app/src/routes/shop/shop.styles.jsx)):
...
Image reliability: a small curated pool of well-known Unsplash photo IDs is rotated across the 36 products by garment type. The product card now has a **two-step image fallback chain** ([`product-card.component.jsx`](../../app/src/components/product-card/product-card.component.jsx)):
...
The new logout-flow guard in [`onboarding.route.jsx`](../../app/src/routes/onboarding/onboarding.route.jsx) force-resets the wizard to step 1 when authentication is lost. Otherwise, the previous user's wizard state in `sessionStorage` could let an unauthenticated user see capture/measurements steps.
