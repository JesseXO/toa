...
app/vercel.json          ← SPA fallback + static caching headers
backend/render.yaml          ← Render Blueprint for web service + Postgres
...
Most likely cause: SSL. Render's Postgres requires SSL but the FE's Sequelize config might not have `dialectOptions: { ssl: { require: true, rejectUnauthorized: false } }`. If you see SSL-related errors in Render logs, add this to `backend/src/models/sequelize.ts`:
