...
|------|---------------|----------|
| Camera capture + TF.js inference | [app/src/routes/onboarding/onboarding.component.jsx](../../app/src/routes/onboarding/onboarding.component.jsx), [app/src/utils/landmark-inference.js](../../app/src/utils/landmark-inference.js) | **REBUILD** — user reports broken. Replace with `react-webcam` + a fresh, smaller TF.js pipeline modeled on [magdazelena/face-landmark-detection](https://github.com/magdazelena/face-landmark-detection). |
| 3-step onboarding (capture → measurements → review) | [app/src/routes/onboarding/onboarding.component.jsx](../../app/src/routes/onboarding/onboarding.component.jsx) | **REBUILD** as a 4-step wizard merged with signup. |
| Body fields on User (heightCm, weightKg, bmi, chestCm, waistCm, hipCm, inseamCm, shoulderCm, bodyShape, preferredFit, landmarkSummary JSONB, onboardingCompletedAt) | [backend/src/models/User.ts](../src/models/User.ts) | **KEEP** — schema is fine; we'll add a few new fields for style. |
| Onboarding endpoints (`POST /me/onboarding/infer`, `PUT /me/body-profile`, `GET /me/body-profile`) | [backend/src/routes/userRoutes.ts](../src/routes/userRoutes.ts) | **KEEP** — payload shape stays compatible; we'll extend with style fields. |
| Size scoring (per-size match against user metrics + preferredFit boost) | [app/src/utils/size-recommendation.js](../../app/src/utils/size-recommendation.js), [backend/src/services/bodySizing.ts](../src/services/bodySizing.ts) | **KEEP** — works, useful. |
| "Fit M" chip on product cards | [app/src/components/product-card/product-card.component.jsx:37](../../app/src/components/product-card/product-card.component.jsx#L37) | **KEEP**, restyle in Phase 7. |
| Theme tokens | [app/src/styles/theme.js](../../app/src/styles/theme.js) | **EXPAND** — single static theme today; add light/dark/alt variants in Phase 4. |
...
**Scope of rebuild:**
- Remove (or stop using) [app/src/routes/onboarding/onboarding.component.jsx](../../app/src/routes/onboarding/onboarding.component.jsx) and the heavy `landmark-inference.js`.
...
- Existing redirect logic in [app/src/App.js:30-48](../../app/src/App.js#L30-L48) stays — it already routes to `/onboarding` when `requiresOnboarding=true`.

**BE work in this phase:** none beyond verifying `POST /me/onboarding/infer` accepts the payload. If it errors, we patch the validator in [backend/src/controllers/user/bodyProfileValidation.ts](../src/controllers/user/bodyProfileValidation.ts).
...
- Add fields to [backend/src/models/User.ts](../src/models/User.ts): `stylePalette JSONB`, `styleBucket STRING`, `styleSilhouette STRING`, `styleConfidence FLOAT`.
- Extend `bodyProfileValidation.ts` whitelist + the resolver.
- Add `paletteHex JSONB` to seed products in [backend/src/seed-data.ts](../src/seed-data.ts) so we have something to match against.
...
- [app/src/components/cart-item/cart-item.component.jsx](../../app/src/components/cart-item/cart-item.component.jsx): show "Recommended: M"; warn softly if `cartItem.size !== recommended.recommendedSize`.
- Same for [app/src/components/cart-dropdown/cart-dropdown.component.jsx](../../app/src/components/cart-dropdown/cart-dropdown.component.jsx).
- [app/src/routes/checkout/checkout.component.jsx](../../app/src/routes/checkout/checkout.component.jsx): summary header — "All sizes match your profile ✓" / "2 items differ".
...
- Backend `GET /products/:id` already exists in [backend/src/controllers/product/getProductByIdResolver.ts](../src/controllers/product/getProductByIdResolver.ts).
...
- **Navigation** ([app/src/routes/navigation/navigation.component.jsx](../../app/src/routes/navigation/navigation.component.jsx)): sticky, glass blur, theme toggle, profile dropdown with body-profile snapshot + "Re-scan" link.
- **Category previews** ([app/src/components/category-preview/category-preview.component.jsx](../../app/src/components/category-preview/category-preview.component.jsx)): hover-lift card grid; "X items match your fit" count.
- **Checkout** ([app/src/routes/checkout/checkout.component.jsx](../../app/src/routes/checkout/checkout.component.jsx)): two-column desktop; sticky summary right; collapsible mobile.
